...
export const updateEmail = asyncHandler(
  async (req: AuthRequest, res) => {
    ...
    const updatedEmail = await prisma.user.update({
      where: { user_id }, data: { email }, select: { email: true },
    });
...
export const updateBio = asyncHandler(
  async (req: AuthRequest, res) => {
    ...
    const updatedBio = await prisma.user.update({
      where: { user_id }, data: { bio }, select: { bio: true }
    })
...
export const updateDp = asyncHandler(
  async (req: AuthRequest, res) => {
    ...
    const updatedImageUrl = await prisma.user.update({
      where: { user_id }, data: { imageUrl }, select: { imageUrl: true }
    })
...
export const resetAccount = asyncHandler(
  async (req: AuthRequest, res) => {
    const user_id = req.user?.user_id;
