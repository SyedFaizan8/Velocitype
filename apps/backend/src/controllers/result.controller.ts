...
export const result = asyncHandler(
    async (req, res) => {
        ...
        try {
            ...
            if (!stats) throw new ApiError(500, "Failed to update the stats after result")
            ...
            const userLeaderboard = await prisma.leaderboard.findUnique({ where: { user_id } })
            ...
            return res
                .status(200)
                .json(new ApiResponse(200, { stats, history }, "Result successfully recorded"))
